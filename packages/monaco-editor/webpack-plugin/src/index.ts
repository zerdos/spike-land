...
function getWorkerFilename(
	...
	return loaderUtils.interpolateName(
		{ resourcePath: entry } as { resourcePath: string },
		filename,
		{
...
class MonacoEditorWebpackPlugin implements webpack.WebpackPluginInstance {
	...
	apply(compiler: webpack.Compiler): void {
		const { languages, features, filename, monacoEditorPath, publicPath, globalAPI } =
			this.options;
...
function addCompilerRules(compiler: webpack.Compiler, rules: webpack.RuleSetRule[]): void {
	...
	if (!compilerOptions.module) {
		compilerOptions.module = { rules: rules } as unknown as typeof compilerOptions.module;
	} else {
