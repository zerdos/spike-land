...
class LspRenameProvider implements monaco.languages.RenameProvider {
	...
	async resolveRenameLocation(
		...
		if ('range' in result && 'placeholder' in result) {
			return {
				range: this._client.bridge.translateBackRange(translated.textDocument, result.range)
					.range,
			...
			const range = this._client.bridge.translateBackRange(
...
function toMonacoWorkspaceEdit(edit: any, client: LspConnection): monaco.languages.WorkspaceEdit {
