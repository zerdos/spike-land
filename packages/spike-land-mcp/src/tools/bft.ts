...
function doPropose(cluster: BftCluster, value: string): ConsensusRound {
  ...
  const prePrepareMsg: PbftMessage = { id: generateMessageId(cluster), type: "pre_prepare", viewNumber: cluster.currentView, sequenceNumber: seqNum, nodeId: leaderId, value, timestamp: Date.now() };
  cluster.messageLog.push(prePrepareMsg);
  leader.phase = "pre_prepare";
  leader.sequenceNumber = seqNum;
  const round: ConsensusRound = { sequenceNumber: seqNum, proposedValue: value, phase: "pre_prepare", decided: false, decidedValue: null, messages: [prePrepareMsg] };
  cluster.rounds.push(round);
  for (const node of cluster.nodes.values()) { node.phase = "pre_prepare"; node.sequenceNumber = seqNum; node.prepareMessages = []; node.commitMessages = []; }
  return round;
}

function doPrepare(cluster: BftCluster, roundSeq: number): ConsensusRound {
  const round = getRound(cluster, roundSeq);
  if (round.phase !== "pre_prepare") throw new Error(`Round ${roundSeq} is in phase "${round.phase}", expected "pre_prepare"`);
  ...
  for (const nodeId of cluster.nodeOrder) {
    ...
    if (node.behavior === "equivocating") {
      const msg: PbftMessage = { id: generateMessageId(cluster), type: "prepare", viewNumber: cluster.currentView, sequenceNumber: roundSeq, nodeId, value: `${round.proposedValue}-EQUIVOC-${nodeId}`, timestamp: Date.now() };
      prepareMessages.push(msg); cluster.messageLog.push(msg); continue;
    }
    const msg: PbftMessage = { id: generateMessageId(cluster), type: "prepare", viewNumber: cluster.currentView, sequenceNumber: roundSeq, nodeId, value: round.proposedValue, timestamp: Date.now() };
    prepareMessages.push(msg); cluster.messageLog.push(msg);
  }
  for (const node of cluster.nodes.values()) { node.phase = "prepare"; node.prepareMessages = [...prepareMessages]; }
...
function doCommit(cluster: BftCluster, roundSeq: number): ConsensusRound {
  const round = getRound(cluster, roundSeq);
  if (round.phase !== "prepare") throw new Error(`Round ${roundSeq} is in phase "${round.phase}", expected "prepare"`);
  ...
  for (const nodeId of cluster.nodeOrder) {
    ...
    if (node.behavior === "silent") continue;
    if (node.behavior === "equivocating") {
      const msg: PbftMessage = { id: generateMessageId(cluster), type: "commit", viewNumber: cluster.currentView, sequenceNumber: roundSeq, nodeId, value: `${round.proposedValue}-EQUIVOC-${nodeId}`, timestamp: Date.now() };
      commitMessages.push(msg); cluster.messageLog.push(msg); continue;
    }
    const matchingPrepares = node.prepareMessages.filter(m => m.value === round.proposedValue).length;
    if (matchingPrepares >= q) {
      const msg: PbftMessage = { id: generateMessageId(cluster), type: "commit", viewNumber: cluster.currentView, sequenceNumber: roundSeq, nodeId, value: round.proposedValue, timestamp: Date.now() };
      commitMessages.push(msg); cluster.messageLog.push(msg);
    }
  }
  for (const node of cluster.nodes.values()) { node.phase = "commit"; node.commitMessages = [...commitMessages]; }
  round.messages.push(...commitMessages);
  round.phase = "commit";
  return round;
}

interface ConsensusResult { decided: boolean; value: string | null; phase: PbftPhase; prepareCount: number; commitCount: number; requiredQuorum: number }

function doCheckConsensus(cluster: BftCluster, roundSeq: number): ConsensusResult {
  const round = getRound(cluster, roundSeq);
  if (round.phase !== "commit") throw new Error(`Round ${roundSeq} is in phase "${round.phase}", expected "commit"`);
  const n = cluster.nodeOrder.length;
  const q = quorumSize(n);
  const prepareCount = round.messages.filter(m => m.type === "prepare" && m.value === round.proposedValue).length;
  const commitCount = round.messages.filter(m => m.type === "commit" && m.value === round.proposedValue).length;
  const decided = commitCount >= q;
  if (decided) {
    round.decided = true; round.decidedValue = round.proposedValue; round.phase = "decided";
    for (const node of cluster.nodes.values()) {
      const nodeMatchingCommits = node.commitMessages.filter(m => m.value === round.proposedValue).length;
      if (nodeMatchingCommits >= q && node.behavior === "honest") { node.decidedValue = round.proposedValue; node.phase = "decided"; }
    }
  }
  return { decided, value: decided ? round.proposedValue : null, phase: round.phase, prepareCount, commitCount, requiredQuorum: q };
}

// ─── Registration ────────────────────────────────────────────────────────────

const BehaviorEnum = z.enum(["honest", "silent", "equivocating"])
...
export function registerBftTools(
  ...
  registry.registerBuilt(
    freeTool(userId, db)
      .tool("bft_create_cluster", "Create a PBFT cluster with N nodes (all honest initially). Requires N >= 4.", {
      ...
      .handler(async ({ input }) => {
        ...
        for (let i = 1; i <= input.node_count; i++) {
          const nodeId = `node-${i}`;
          nodes.set(nodeId, { id: nodeId, behavior: "honest", phase: "idle", viewNumber: 0, sequenceNumber: 0, decidedValue: null, prepareMessages: [], commitMessages: [] });
          nodeOrder.push(nodeId);
        }
        const cluster: BftCluster = { id, userId, name: input.name, nodes, nodeOrder, rounds: [], currentView: 0, currentSequence: 0, messageLog: [], messageCounter: 0, createdAt: Date.now() };
