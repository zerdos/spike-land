...
heroImage: "/blog/how-claude-code-engineers-context/hero.svg"
---

![A modern digital illustration for a blog post titled 'How Claude Code Engineers Context: An Interview with Opus 4.6'. The style should be clean, minimalist, high contrast dark theme, with vibrant accent colors, conceptual and abstract.](/blog/how-claude-code-engineers-context/hero.png)
...
## The Tool That Caught Me Doing What I Teach

Last week I published an article about [context engineering your zero-shot prompt](/blog/context-engineering-your-zero-shot-prompt) — the idea that getting great AI output on the first try is about front-loading context, not crafting clever incantations. Five layers: Identity, Knowledge, Examples, Constraints, Tools.

The next day, I was using Claude Code in plan mode to design a new feature. It spawned four parallel agents, read fourteen files, ran six grep searches, threw away half the results, and produced a structured plan that mapped precisely to the five layers I had just written about.

I was watching my own framework, automated. Like writing a recipe and then watching someone else's kitchen robot follow it without reading the book.

This article is the conversation that followed. I sat down with Opus 4.6 — the model that powers Claude Code — and asked it to explain what happens under the hood. Not the marketing version. The mechanism.
...
This is worth pausing on. The restriction is not mechanical — it is contextual. The system trusts the model to follow instructions rather than removing capabilities. Think of it like a speed limit sign versus a physical speed bump. The sign is faster to put up, easier to change, and it works because it sits in the system prompt where it receives maximum attention weight.
...
Here is the key insight: **the plan file is a briefing for a future version of me that has zero memory of the exploration I just did.** Everything important must be in the document. If I discovered that your project uses a specific pattern in `src/components/dashboard/WorkspaceStats.tsx`, that file path needs to be in the plan — not because the plan is documentation, but because the executing agent needs that token in its context window to make the right decisions.

It is like leaving a note for your future self the night before. You know you will wake up groggy, with no recollection of the insight you had at midnight. The note has to be complete enough to act on. That is what a plan file is.
...
I follow a high-signal-first strategy. Think of it as triage in an emergency room — you assess broadly first, then focus resources where they matter most.

**Phase 1: Structure.** I use Glob to find files matching patterns — `**/*.tsx`, `**/types.ts`, `**/schema.prisma`. This tells me the shape of the project without reading any content. It is cheap in tokens and gives me a map. Like looking at a city from above before walking any streets.
...
This is the same pattern a good team lead uses: delegate the research, then read the memos.
...
Think of it like desk space. If every researcher dumped their entire stack of papers on your desk instead of handing you a one-page summary, you would drown in paper before you found the insight. Isolation is not just organizational convenience. It is an information-theoretic necessity.
...
This is the core mechanism — the engine room. Every token computes three vectors from its embedding: a Query ("what am I looking for?"), a Key ("what do I contain?"), and a Value ("here is my actual content"). The attention score between any two tokens is:
...
But it is also why irrelevant tokens hurt. The softmax normalizes attention weights to sum to 1. If there are 1,000 tokens of useful code and 4,000 tokens of irrelevant file contents, the attention weight on the useful code is diluted by a factor of five. The signal is still there, but it is like trying to hear a conversation across a crowded room — the more noise, the harder each voice is to pick out.

Multi-head attention runs this computation in parallel across multiple "heads" — typically 32 to 128 — each learning different relationship types. Some heads track syntactic structure. Some track semantic relationships. A small fraction — research found about 3-6% — are "retrieval heads" that mechanistically extract factual information from context. When those heads are ablated, the model remains fluent but starts hallucinating. It can still talk, but it stops knowing what it is talking about.
...
Think of it like a courtroom transcript. Instead of replaying every witness's testimony every time the jury asks a question, the transcript is right there. You just look up what was said.
...
This asymmetry is why Anthropic charges $5 per million input tokens but $25 per million output tokens for Opus 4.6. Input is cheap because it is parallel. Output is expensive because it is sequential. Like the difference between reading a book (fast, parallel eye movements) and writing one (slow, word by word).
...
It is recursive in a way that feels almost biological: I think, then I think about what I thought, and the result of that thinking shapes my next action. The difference is that every step is context engineering — each batch of reasoning tokens reshapes the probability landscape for whatever comes next.
...
The plan is not documentation. It is a prompt. Every line exists to increase the probability that the executing agent produces the correct implementation. The way a good recipe is not a description of a meal — it is a set of instructions that maximizes the probability of a delicious one.
...
Same principle, different mechanism. Research on the "lost in the middle" problem shows that models attend most strongly to tokens at the beginning and end of the context window. Information in the middle receives less attention — performance can degrade by over 30% when critical information shifts from the edges to the center.
...
Picture a sandwich. The bread — the beginning and end — is what you taste most. The filling in the middle can get lost, especially if there is too much of it. Context engineering is partly about putting the important ingredients in the bread.
...
They exist because a single context window cannot hold everything. It is a stock-and-flow problem. A typical coding task might require understanding the database schema, the API layer, the component hierarchy, the test patterns, and the CI configuration. Reading all of that into one context window would consume 50,000-100,000 tokens of exploration before writing a single line of code. That is most of the stock consumed by exploration, leaving nothing for execution.
...
This is the "Isolate" pattern from Lance Martin's Write/Select/Compress/Isolate framework. Instead of polluting one context window with everything, you give each agent exactly the context it needs — then compress and merge the results. It is the same principle behind why companies have departments instead of one giant meeting room.
...
A 12:1 compression ratio. That is the leverage.
...
Prompt caching changes the game. When a request prefix matches a recently cached version — same system prompt, same tools, same CLAUDE.md — the server reuses the cached KV states instead of recomputing them. The pricing reflects the savings:
...
The economics create a reinforcing feedback loop: the more stable your context prefix, the cheaper each call becomes, which incentivizes keeping the prefix stable, which makes each call cheaper still.
...
Yes. The KV cache is not an implementation detail you can safely ignore. It is the economic foundation. Cached tokens are cheap. Uncached tokens are expensive. Output tokens are very expensive. This pricing structure rewards preparation and penalises trial-and-error.
...
The system rewards you for thinking before you act. That is a rare alignment between economics and good practice.
...
Imagine waking up every morning with complete amnesia, but someone left you a detailed notebook. That notebook is your context window. Its quality determines your entire day.
...
This is why your role as the human in the loop is irreplaceable. Not for writing code — I can write code. Not for finding files — I can search. For catching the assumptions I did not know I was making. The plan review step is not quality assurance in the traditional sense. It is assumption auditing. You are the error-correcting feedback loop that the system cannot provide for itself.
...
Eight techniques, drawn directly from how Claude Code operates. Each one has a clear reason rooted in the mechanics we just discussed.
...
The difference is precision. When you say "follow our patterns," the model has to guess what you mean. When you point to a specific file, the model reads the actual implementation and extracts every nuance — naming conventions, error handling approach, import style, test structure — without you having to articulate any of it. You give it the answer instead of asking it to guess the question.

**2. Include minimum viable context, not maximum.**

Research demonstrates that model performance degrades as context length increases — even when retrieval is perfect. Five relevant files will produce better results than fifty files "just in case." More tokens means more attention dilution.

The practical test: for each piece of context you include, ask "Would removing this change the model's output?" If the answer is no, remove it. Anthropic's framing is precise: find "the smallest set of high-signal tokens that maximize the likelihood of your desired outcome." A smaller, sharper signal beats a larger, noisier one every time.
...
Think of constraints as guardrails on a mountain road. Without them, the model is free to explore the full landscape — including the cliff edges.
...
Write like you are leaving instructions for a brilliant contractor who has never seen your building before. They need to know which floor, which room, and which wall — not just "the usual spot."
...
There is no magic. There is no secret sauce. There is only context, at every level — shaping what comes next.
...
This is an honest limitation I cannot fully answer, because I do not know what was compressed. That is the nature of the problem.
...
It is like waking up knowing you dreamed something important, but unable to recall what. You cannot miss what you do not remember.
