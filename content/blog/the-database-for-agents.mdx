...
heroImage: "/blog/the-database-for-agents/hero.svg"
...
But tools need data. And data needs a backbone. This post is about the backbone — how I found it, lost it, and rebuilt it in a way that taught me something about the difference between good patterns and good infrastructure.
...
- **Tables** as schema — define them in a module, they hold your data
...
Then on March 3, I sat down and asked myself a question that should have come first: *what does SpacetimeDB give me that I do not already have?*

The answer was humbling: patterns. Just patterns. The infrastructure underneath — D1, KV, R2, Durable Objects — was already sitting in my Cloudflare account. I had been paying for it. I had been deploying to it. I just had not organized my code the way SpacetimeDB organized its modules.

It was like buying a fancy new bookshelf and then realizing you already own one — you just never assembled it.
...
The spike.land platform was a Next.js monolith. Around 520 routes. Around 383 API endpoints. 120+ MCP tool files. It worked. And it had all the problems that working monoliths accumulate over time.

**Schema drift.** Prisma migrations are a ceremony. The schema lives in `schema.prisma`. The types live in `@prisma/client`. The validation lives in Zod schemas scattered across route handlers. Every schema change touches three places. Miss one and you get a runtime error that the compiler blessed. It is like having three copies of a map and hoping they all agree on where the roads are.

**Polling hell.** When 50 agents want to know if a task is available, that is 25 requests per second doing nothing useful. Redis pub/sub helps, but now you have two data models that drift apart.

**Auth ceremony.** NextAuth gives you session tokens, JWT refresh, middleware chains. Every MCP tool needs to verify the caller. That is 120+ tools importing the same auth middleware, and every one of them is a place where a mistake hides.

**No coordination primitive.** Agents can call tools. Agents cannot talk to each other. Every coordination mechanism — Redis pub/sub, SQS — is a bolt-on service with its own deployment and failure modes.

I tried vinext too. It was still Next.js underneath, just with a different build pipeline. The architectural friction was the same: a monolith pretending to be modular, like a warehouse with painted lines on the floor pretending to be separate rooms.
...
The claim workflow is atomic: `UPDATE ... WHERE status = 'pending'` means only one agent can claim a task. No distributed locks. No Redis. Just SQL doing what SQL has always done well — enforcing constraints at the moment of write.
...
**No built-in real-time subscriptions.** SpacetimeDB's subscription queries — where you subscribe to a SQL query and get deltas pushed automatically — were genuinely great. On CF Workers, real-time requires Durable Objects + WebSocket plumbing. It works, but it is not a primitive. You have to build the pipe yourself.
...
**Vendor lock-in shifted.** From SpacetimeDB (small startup, young ecosystem) to Cloudflare (massive company, mature ecosystem). I sleep better at night, but I am still locked in. The `StorageAdapter` abstraction means I *could* swap backends, but let us be honest — if Cloudflare disappeared tomorrow, I would be rewriting more than adapters.

**block-sdk is custom and young.** No community. No ecosystem. No Stack Overflow answers. If I get hit by a bus, the next developer reads my code or starts over. But: I own every line. I can read every line. I can change it on a Tuesday. At this scale, that matters more than ecosystem size.
...
**SpacetimeDB's developer experience was genuinely good.** The Rust module system, the auto-generated bindings, the single-binary deployment — these are well-designed. If SpacetimeDB had been a Cloudflare product, I might still be using it. The reason I left was not quality. It was that I was adding a dependency I did not need on top of infrastructure I already had. Like buying a car when you already own one and the roads are the same.

## Closing

The best architecture is not always the most novel one. Sometimes it is the one you built by stealing patterns from something that impressed you and rebuilding them on infrastructure you already understood.
