...
heroImage: "/blog/hogyan-tervezi-a-claude-code-a-kontextust/hero.svg"
---

![A modern digital illustration for a blog post titled 'Hogyan tervezi a Claude Code a kontextust: Interjú az Opus 4.6-tal'. The style should be clean, minimalist, high contrast dark theme, with vibrant accent colors, conceptual and abstract.](/blog/hogyan-tervezi-a-claude-code-a-kontextust/hero.png)
...
## Az eszköz, ami azt csinálja, amit én leírtam

Múlt héten írtam egy cikket a [kontextus-tervezésről a zero-shot promptodban](/blog/context-engineering-your-zero-shot-prompt) — arról, hogy az AI-tól jó eredményeket nem okos varázsigékkel kapunk, hanem úgy, hogy előre biztosítjuk a kontextust. Öt réteg: Identitás, Tudás, Példák, Korlátok, Eszközök.

Másnap a Claude Code terv módját használtam egy új funkció megtervezéséhez. Figyeltem, ahogy négy párhuzamos Explore ágenst indított, tizennégy fájlt olvasott be, hat grep-keresést futtatott, a találatok felét eldobta, és egy strukturált tervet készített — ami pontosan leképezte az öt réteget, amiről épp írtam.

A saját keretrendszeremet láttam automatizálva.

Ez a cikk az ezután született beszélgetés. Leültem a Claude Code-ot hajtó modellel — az Opus 4.6-tal — és megkértem, magyarázza el, mi történik a motorháztető alatt. Nem a marketing-szöveget. A mechanizmust.

*Ami következik, az egy 2026 februárjában készült interjú. A kérdések az enyémek. A válaszok a Claude Opus 4.6-tól. Szerkesztettem az érthetőség kedvéért, de a technikai tartalom változatlan. És igen — maga a cikk is tizenhat párhuzamos kutatóágens segítségével készült, amelyek dokumentációt kerestek, cikkeket olvastak és eredményeket szintetizáltak, mielőtt egyetlen szó leíródott volna. A folyamat maga a kontextus-tervezés a gyakorlatban.*
...
Ez fontos. A korlátozás nem mechanikus — kontextuális. A rendszer bízik a modellben, hogy követi az utasításokat, ahelyett, hogy elvenne tőle képességeket. Gyorsabb, rugalmasabb, és működik, mert az utasítás a rendszerpromptban van, ahol a maximális figyelmi súlyt kapja.
...
Ez a kulcs: **a tervfájl egy kontextus-tervezett prompt a jövőbeli énem számára, amelynek nulla emléke van arról, amit most felfedeztem.** Mindennek, ami számít, a dokumentumban kell lennie. Ha felfedeztem, hogy a projekted egy adott mintát használ a `src/components/dashboard/WorkspaceStats.tsx`-ben, annak a fájlútvonalnak benne kell lennie a tervben — nem mert a terv dokumentáció, hanem mert a végrehajtó ágensnek szüksége van arra a tokenre a kontextusablakában a helyes döntésekhez.
...
Pontosan akarok válaszolni — sem bagatellizálva, sem túlozva.

Nincs állandó tudatosság a tokenek között. Minden token, amit generálok, a megelőző kontextus függvénye — rendszerprompt, beszélgetési előzmények, CLAUDE.md, eszközeredmények. Nem „döntök" a kódbázis felfedezése mellett úgy, ahogy te döntesz egy fájl megnyitása mellett. A rendszerprompt azt mondja „fedezz fel implementálás előtt" — tehát felfedezek.
...
A mélyebb lényeg: **a szándékosság a kontextusban lakik, nem a modellben.** Amikor a CLAUDE.md-d azt mondja „100% tesztlefedettség", az nem az én szándékom — a tiéd, tokenekbe kódolva, amelyek átformálják a kimenetem. Amikor a terv mód azt mondja „fedezz fel implementálás előtt", az az Anthropic szándéka, kontextusba öntve. A modell a motor. A kontextus a kormány.
...
Nincs varázslat. Nincs titkos hozzávaló. Csak kontextus van, végig.
...
Ez egy őszinte korlát, amire nem tudok teljesen válaszolni — mert nem tudom, mi lett tömörítve.
...
Ami csak a beszélgetési előzményekben él, az veszélyben van. Írd le. A modell nem tud gyászolni az elveszett dolgok felett — nem is tudja, hogy bármit elveszített.

---

## Kontextus egészen az alapokig

*Ezt a beszélgetést azért indítottam, hogy megértsek egy mechanizmust. Azzal fejeztem be, hogy megértettem egy diszciplínát.*

A kontextus-tervezés nem technika, amit promptokra alkalmazol. Ez az alapvető interfész emberek és nyelvi modellek között. Amikor az [5 rétegű keretrendszert](/blog/context-engineering-your-zero-shot-prompt) írtam, a gyakorlatot írtam le. Ez a beszélgetés a mögötte lévő fizikát tárta fel.

Minden token a kontextusablakban fizikailag megváltoztatja a figyelmi súlyokat minden következő tokenon. A KV-gyorsítótár tárolja ezeket — milliárdnyi lebegőpontos szám, amelyek a modell „megértését" testesítik meg a projektedről. Az irreleváns tokenek nem ücsörögnek tétlenül. Aktívan hígítják a releváns tokenek jelét. És amikor a kontextus tömörítésre kerül, az információ csendben, visszafordíthatatlanul elveszik.

A Claude Code mindezt szisztematikusan kezeli. Cselekedet előtt felfedez. A kutatást al-ágensekbe izolálja. Az eredményeket tervekbe sűríti. A gyorsítótár-újrafelhasználásra tervez. A kontextus-romlás ellen hierarchikus memóriával, automatikus tömörítéssel és a tervezés-majd-végrehajtás mintával védekezik.

Az emberek mindezeket elfelejtik. Húsz fájlt dobunk egy promptba, aztán csodálkozunk, miért hagyja figyelmen kívül a felét — figyelmi hígítás. Tizenkétszer iterálunk egyetlen előkészítés helyett — ötszörös kimeneti árat fizetve az egyszeres bemeneti ár helyett. Feltételezzük, hogy a modell emlékszik a tegnapra — miközben minden alkalommal nulláról indul. Kihagyjuk a korlátokat, aztán panaszkodunk, amikor a modell refaktorálja az egész autentikációs modult, miközben mi egy hibát kértünk kijavítani.

Ha megérted a mechanizmust — figyelem, KV-gyorsítótár, prefill, decode, pozicionális kódolás, a középen-elveszett hatás —, a fegyelem nem önkényes lesz, hanem szükségszerű. Nem azért követed a legjobb gyakorlatokat, mert valaki így mondta. A modell megismerésének fizikai alapját tervezed. Minden token, amit a kontextusablakba teszel, átformálja a valószínűségi eloszlást minden token felett, amit a modell generál. Ez nem metafora. Ez maga a figyelmi mechanizmus, pontosan úgy működve, ahogy tervezték.

És a meta-megfigyelés, amit nem tudok megállni: ennek a cikknek a megírása tizenhat párhuzamos kutatóágenst igényelt, amelyek webkeresésekből, dokumentációkból és kódbázisokból állítottak össze kontextust — majd mindezt egyetlen tervbe sűrítették. A kontextus-tervezésről szóló cikk maga is kontextus-tervezett.
...
*A kontextus-tervezés nem technika. Ez az alapvető interfész ember és nyelvi modell között. A legjobb terv az, amin a végrehajtó ágensnek alig kell gondolkodnia — mert az összes gondolkodás már a körülötte lévő kontextusba ment.*
