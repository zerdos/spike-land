...
*Your AI is not writing buggy code. Your process is writing buggy specs.*
...
CodeRabbit's [2025 code quality study](/learnit/coderabbit-ai-code-quality-defect-study-2025) found that AI-assisted pull requests carry **1.7x more defects** than human-written ones. Not because the AI writes worse syntax — it writes *more* code, faster, with less review. Volume without verification. Like a factory that doubled its output by turning off quality control.
...
The pattern repeats. GitClear's code churn analysis shows refactoring activity declining year over year as AI adoption rises. Teams are generating more code but maintaining less of it. The codebase grows. The understanding shrinks. It is the software equivalent of a city building roads faster than it can maintain them — eventually everything is potholed.
...
Every minute spent on a spec saves ten minutes of debugging. Every hour of planning saves a day of rework. This is not a productivity hack. It is project management — the old-fashioned kind that works. The question is whether you are willing to apply it to your AI workflow the same way you apply it to your human one.
...
The top of the hourglass is wide: E2E specs that describe what the user experiences. "Given a logged-in user, when they change their email and confirm, then the profile shows the new email." These are written by humans. They are the requirements — the stock of accumulated decisions that define what the system should do.
...
> The pyramid was built for typing. The hourglass is built for thinking.

But *where* inside the hourglass does each test belong? The ratios are where this saves you hours or costs you them.
...
You need far fewer of these than you think. If the MCP tool test proves that cancellation with reason "too_expensive" works, the E2E test only needs to verify that the button click reaches the tool with the right arguments. The heavy lifting is already done.
...
Knowing where tests live is half the job. The other half is making sure the right test gets written at all — which is where planning earns its keep.
...
Planning, solved. Testing, solved. Now something has to run it — fast enough that the feedback loop never breaks.
...
**Cost economics.** A monolithic E2E suite running 200 Playwright tests on every PR costs roughly 15-20 minutes of CI compute. With hourglass CI, the same PR runs 15 unit tests (5 seconds), one build (60 seconds), and 2-3 targeted E2E tests (30 seconds). Total: under 2 minutes. That is not incremental improvement — it is a category change in developer feedback speed.

> Hourglass CI does not make tests faster. It stops punishing you for iterating.

**Blast radius analysis.** When a PR modifies a business rule — say, changing the cancellation policy from "end of period" to "immediate" — the hourglass CI catches it in three places. The MCP tool test fails because the expected `effectiveDate` changed. The build might fail if the component references the old field. The E2E test fails if the confirmation message is wrong. Three independent signals, all within 2 minutes. The system triangulates the breakage the way GPS triangulates position — multiple weak signals combining into a precise location.
...
Which leaves one thing. The part no pipeline can run for you.
...
Decide the user flow before the AI writes the component. Decide the business rules before the AI writes the handler. Decide the acceptance criteria before the AI writes the test. The decisions are the hard part. The generation is the easy part. This is the inversion that AI makes visible: the bottleneck was never typing speed. It was always thinking speed.
