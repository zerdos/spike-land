...
I thought the feeling would go away. It did not. So I started paying attention to it instead.

Here is what I found.

The feeling has a name. It is the sound of a paradigm shift arriving before the culture is ready for it. When something feels too easy, it is not because you are cheating. It is because you eliminated a category of work that used to be hard. The guilt is your old identity protesting. Your brain is comparing your output against the amount of suffering it expects, and the numbers do not match.

Like someone who grew up hand-washing clothes getting their first washing machine. The clothes are clean. The process took ten minutes. And yet some part of them feels like it does not count.
...
I had a test that clicked a button, waited for a modal, filled in three fields, clicked submit, waited for a toast notification, and then checked a database value. The test was sixty lines long. Fifty-five of those lines were about the browser. Five were about the business logic.

That ratio stopped me cold.

Fifty-five lines of ceremony. Five lines of substance. I was testing the frame instead of the painting.

I asked myself: what if I just tested the five lines?

So I moved all the business logic into MCP tools. Every piece became a typed function with a Zod schema, a handler, and a structured response. Input validated. Output typed. Contract enforced.
...
I sat there staring at my screen. I had spent weeks writing Playwright tests that tested the browser's ability to click buttons. The actual business logic — the part that mattered — was tested as a side effect. An afterthought.

When I realized that, I said a word in Hungarian that I will not translate here.

## The Hourglass

The testing pyramid was always the wrong shape for this era.
...
AI changed the economics. When code is cheap to generate, what you test and how you test it has to change too.
...
The top layer is E2E specs written by humans. Given the user is logged in, when they click checkout, then the order is placed. These describe *what should happen*. They are requirements in executable form.

The bottom layer is MCP tool tests. Business logic. Validation rules. State transitions. Authorization. Contract enforcement. These prove the system *actually works*. They run in milliseconds. They never flake.
...
The middle is disposable because of a simple systems principle: if you know *why* the code exists (top) and you can *prove* it works (bottom), the code itself is just a rendering. An artifact. A printout. The value lives in the requirements and the tests. The code is just what happens when you run them.
...
**Agent dev tools.** Claude Code uses them during development. The agent can call `file_guard` to pre-check a file change before committing. It can run the exact MCP tool it just built to verify the output matches the schema. It can explore database state, check auth flows, validate business rules — all through the same interface the production app uses.

This is why Claude Code goes three to five times faster on this codebase.

The agent is not writing code blindly and hoping CI catches mistakes twenty minutes later. It has surgical instruments. It can test its own work in real time. It can verify a business rule before writing the component that calls it.

It is like giving a surgeon an MRI machine in the operating room instead of making them wait for results next Tuesday.

When the tools serve three purposes simultaneously, you do not have three codebases to maintain. You have one. And that one codebase is the simplest version of the system, because every tool is just a function with a typed schema and a handler.

## The Infinitely Fast CI Server

Someone told me once: "You get an infinitely fast CI server." I did not understand what they meant at the time.

Now I do.

Think about what happens when CI is slow. When the test suite takes thirty minutes, your bug has already compounded. By the time you learn about it, you have built three more features on top of broken foundations. Now you are doing archaeology. Which of these forty-seven commits broke it? Was it this one? No. This one? Maybe. Ah — it was actually two commits interacting in a way nobody predicted.
...
If your CI pipeline runs in under ten seconds, you can skip branches entirely. Commit directly to main. Trunk-based development. The same pattern Google and Meta use, but instead of a thousand engineers making it work, you have a fast test suite making it work.

The math is simple. Fifty commits a day at five seconds each is four minutes of waiting. Branching overhead at five minutes per change is two hundred fifty minutes of ceremony. Four minutes versus four hours. That is not an optimization. That is a different game.
...
The hourglass makes the speed possible. The speed makes trunk-based development possible. Trunk-based development makes rapid iteration possible. Rapid iteration makes AI development possible.

It is a reinforcing loop. Pull one piece out and the whole thing slows down. Put them all together and you get something that feels like cheating.
...
And yes, the feeling is the same. The exact same cocktail of guilt and exhilaration. The exact same voice saying this cannot be right, this is too easy, I must be missing something.
...
The guilt is the signal. When something feels too easy, it is because you eliminated a category of work that was never the real value. Memory management was never the point. SQL was never the point. Deployment was never the point.
...
The answer: your identity. The idea that your value comes from the code you write. That the act of typing is the act of creating. That the artifact is the art.
...
That is the whole method in one breath. Each one earned the hard way. Each one a response to a specific failure that was experienced, documented, and sworn never to be repeated.
...
What makes me valuable is understanding problems. Describing solutions precisely. Building systems where AI does the heavy lifting while I do the thinking. Knowing what to build, why to build it, and how to prove it works.
