---
title: "The Architecture of Scale: How I Refactored 30+ AI Agents to Build Their Own Tools"
slug: "the-architecture-of-scale-building-tools"
description: "When Google released Nano Banana 2, my image tools became obsolete overnight. Here's how I rebuilt my MCP architecture so autonomous agents could write and deploy their own tools."
date: "2026-03-01"
author: "Zoltan Erdos"
...
I have been spiking on ESM, TypeScript, and build pipelines for five years. Not at a company. Not on a team. Just me, a mass of repositories, and an increasing conviction that the way we wire AI tools together is fundamentally wrong.
...
Something I did not plan but should have predicted: the handler signature `(input, ctx)` is the same pattern Cloudflare Workers uses. And tRPC. And Hono. And most modern serverless frameworks.
...
No globals. No `this`. No ambient state. Pure functions that take input and context and return a result. The function does not know where it runs. It does not know who called it. It does not care.

This is not a coincidence. When you strip away framework ceremony, every request handler in every system does the same thing: receive input, use context, produce output. The only question is how much ceremony the framework wraps around that core operation.

My old `ToolBuilder` class added ceremony. The new builder removes it. What remains is the pattern that Cloudflare, Vercel, and the tRPC community independently converged on — because it is the minimal viable abstraction for a request handler.
...
Five years ago I started building what would become spike.land. I did not know it would become an MCP platform. I did not know Google would release a model that obsoleted half my tools on a Tuesday. I did not know I would need autonomous agents to write their own tools.

What I did know was that [every abstraction I reached for felt wrong](/blog/context-engineering-your-zero-shot-prompt). Too heavy. Too coupled. Too many assumptions about how the code would be used. So I kept spiking. ESM bundling. TypeScript strict mode everywhere. Zod schemas for everything. Dependency injection instead of imports. Pure functions instead of classes.

None of those decisions felt important at the time. They were just preferences. Habits. The kind of thing that makes your codebase slightly annoying to set up but slightly easier to change.
