...
heroImage: "/blog/the-testing-pyramid-is-upside-down/hero.svg"
---
...
The presenter was young. Nervous energy. He had clearly been converted recently — you could see it in his eyes, the way people look when they have just discovered something that rearranges their world. He walked us through how a few dozen unit tests had caught a regression that would have shipped to production. He showed how mocking worked. He showed how fast the feedback loop was. He was practically vibrating.
...
But Cypress was the wrong direction. Not because the tool was bad — the tool was excellent. Because the premise was wrong. We were perfecting the art of driving a browser when we should have been removing the need for one.

Think of it like this: imagine you want to verify that a letter was delivered. You could build an elaborate system that follows the postal truck, watches it stop at the house, films the postman walking to the door. Or you could just check the mailbox. Browser automation is the postal truck surveillance camera. We needed the mailbox.
...
They are slow. A fast unit test suite runs in seconds. A comprehensive E2E suite runs in minutes, often tens of minutes. On CI, with parallelisation and retries, you are looking at pipeline times that make developers switch to another task while they wait — and that context switch is where bugs hide.

They are flaky. Not because the tools are bad, but because browsers are complex state machines with thousands of moving parts. A test that passes locally fails on CI because an animation took 50ms longer. A test that ran fine yesterday fails today because a third-party script loaded slower. You add `waitForSelector`. You add `waitForTimeout`. You add retry logic. You are not testing your application anymore — you are negotiating with chaos.
...
Think about what a typical E2E test actually verifies. "User logs in, navigates to settings, changes their email, confirms the change, sees the updated email on the profile page." What are you really testing? The email change flow. The browser is just the delivery mechanism — like testing whether a restaurant's food is good by evaluating the quality of the plates it is served on.
...
A plain service layer gives you one of these. MCP gives you three. The same artifact is your test contract, your agent interface, and your feature documentation. You write it once; it pays out three times. That is not clever architecture — it is a leverage point, a single intervention that produces outsized results because it sits at the intersection of three needs.
...
**Step 2: Ask what business logic they actually verify.** Strip away the clicks and the waits and the selectors. What is the test really checking? "User can cancel their subscription." "Admin can ban a user." "Payment flow handles declined cards." That is the business logic — the load-bearing decisions underneath the pixels.

**Step 3: Expose that logic as MCP tools.** Write an MCP tool for each business flow. Define the input schema, implement the handler using your existing services, return structured results. You are not rewriting anything — you are wrapping your existing business logic in a structured interface, the way a power socket wraps the wiring behind it.
...
You are not replacing E2E tests. You are moving the business logic out of them. What remains is a thin layer of visual smoke tests — the only job E2E tests should have ever owned.
...
`update_user_email` changed? Run the email E2E scenarios. `list_user_notifications` unchanged? Skip those. Updated a test fixture description? No E2E tests needed. The MCP tool names give the agent enough semantic context to reason about blast radius — the ripple pattern of what a change might break.

The savings compound. On a large codebase, a typical PR touches a fraction of the business logic. Running the full E2E suite for every PR is like repainting the entire house because you touched up one wall. With MCP-structured business logic and an AI reviewer, your CI runs only what matters.
...
The remaining E2E tests do what they should have always done: verify that the page renders, that the integration wiring holds, that the visual design is correct. A handful of Playwright smoke checks, not a suite of browser-driven business logic simulations.

"Az UI-t? Azt teszteljék a hülyék!" — *"The UI? Let the idiots test that!"*

It turns out that was always the right answer. We just needed twenty years and a protocol to understand what it actually meant.

---

## Full Circle

That conference never left me. The young presenter, vibrating with excitement about unit tests. The person in the audience asking about UI testing. The dismissive answer that was not quite wrong.
...
The real answer to the UI testing question was never "automate the browser." The real answer was: make the business logic accessible without one. We did not have the interface for it yet. Now we do.
