...
function isIndexed(maybeIndexed) {
  return Boolean(
    maybeIndexed && // @ts-expect-error: maybeIndexed is typed as `{}`, need to change in 6.0 to `maybeIndexed && typeof maybeIndexed === 'object' && IS_INDEXED_SYMBOL in maybeIndexed`
...
function isKeyed(maybeKeyed) {
  return Boolean(
    maybeKeyed && // @ts-expect-error: maybeKeyed is typed as `{}`, need to change in 6.0 to `maybeKeyed && typeof maybeKeyed === 'object' && IS_KEYED_SYMBOL in maybeKeyed`
...
function isCollection(maybeCollection) {
  return Boolean(
    maybeCollection && // @ts-expect-error: maybeCollection is typed as `{}`, need to change in 6.0 to `maybeCollection && typeof maybeCollection === 'object' && IS_COLLECTION_SYMBOL in maybeCollection`
...
function isIterator(maybeIterator) {
  return !!(
    maybeIterator && // @ts-expect-error: maybeIterator is typed as `{}`
...
function getIteratorFn(iterable) {
  var iteratorFn =
    iterable && // @ts-expect-error: maybeIterator is typed as `{}`
...
function isRecord(maybeRecord) {
  return Boolean(
    maybeRecord && // @ts-expect-error: maybeRecord is typed as `{}`, need to change in 6.0 to `maybeRecord && typeof maybeRecord === 'object' && IS_RECORD_SYMBOL in maybeRecord`
...
function isOrdered(maybeOrdered) {
  return Boolean(
    maybeOrdered && // @ts-expect-error: maybeOrdered is typed as `{}`, need to change in 6.0 to `maybeOrdered && typeof maybeOrdered === 'object' && IS_ORDERED_SYMBOL in maybeOrdered`
...
function isSeq(maybeSeq) {
  return Boolean(
    maybeSeq && // @ts-expect-error: maybeSeq is typed as `{}`, need to change in 6.0 to `maybeSeq && typeof maybeSeq === 'object' && MAYBE_SEQ_SYMBOL in maybeSeq`
...
function isArrayLike(value) {
  ...
  return (
    value &&
    typeof value === "object" && // @ts-expect-error check that `'length' in value &&`
...
var ToIndexedSequence = /* @__PURE__ */ (function (IndexedSeq2) {
  ...
  ToIndexedSequence2.prototype.__iterator = function __iterator2(type, reverse3) {
    ...
    return new Iterator(function () {
      ...
      return step.done
        ? step
        : iteratorValue(type, reverse3 ? this$1$1.size - ++i : i++, step.value, step);
    });
  };
...
function mapFactory(collection, mapper, context) {
  ...
  mappedSequence.__iterateUncached = function (fn, reverse3) {
    var this$1$1 = this;
    return collection.__iterate(function (v, k, c2) {
...
var ConcatSeq = /* @__PURE__ */ (function (Seq2) {
  ...
  ConcatSeq2.prototype.__iterateUncached = function __iterateUncached(fn, reverse3) {
    ...
    var currentIterator = this._wrappedIterables[iterableIndex].__iterator(iteratorType, reverse3);
    var keepGoing = true;
    var index = 0;
    while (keepGoing) {
      ...
      while (next.done) {
        ...
        currentIterator = this._wrappedIterables[iterableIndex].__iterator(iteratorType, reverse3);
  ...
  ConcatSeq2.prototype.__iteratorUncached = function __iteratorUncached(type, reverse3) {
    ...
    var iterableIndex = 0;
    var currentIterator = this._wrappedIterables[iterableIndex].__iterator(type, reverse3);
    return new Iterator(function () {
      var next = currentIterator.next();
      while (next.done) {
        iterableIndex++;
        ...
        currentIterator = this$1$1._wrappedIterables[iterableIndex].__iterator(type, reverse3);
        next = currentIterator.next();
      }
      return next;
    });
...
function isValueObject(maybeValue) {
  return Boolean(
    maybeValue && // @ts-expect-error: maybeValue is typed as `{}`
...
function isMap(maybeMap) {
  return Boolean(
    maybeMap && // @ts-expect-error: maybeMap is typed as `{}`, need to change in 6.0 to `maybeMap && typeof maybeMap === 'object' && IS_MAP_SYMBOL in maybeMap`
...
function isList(maybeList) {
  return Boolean(
    maybeList && // @ts-expect-error: maybeList is typed as `{}`, need to change in 6.0 to `maybeList && typeof maybeList === 'object' && IS_LIST_SYMBOL in maybeList`
...
function isStack(maybeStack) {
  return Boolean(
    maybeStack && // @ts-expect-error: maybeStack is typed as `{}`, need to change in 6.0 to `maybeStack && typeof maybeStack === 'object' && MAYBE_STACK_SYMBOL in maybeStack`
...
function deepEqual(a, b) {
  ...
  if (
    ...
    (a.__hash !== void 0 && // @ts-expect-error __hash exists on Collection
      ...
      a.__hash !== b.__hash) ||
    isKeyed(a) !== isKeyed(b) ||
    isIndexed(a) !== isIndexed(b) || // @ts-expect-error Range extends Collection, which implements [Symbol.iterator], so it is valid
  ...
  return (
    allEqual && // @ts-expect-error size should exists on Collection
    a.size === bSize
  );
}
...
function isSet(maybeSet) {
  return Boolean(
    maybeSet && // @ts-expect-error: maybeSet is typed as `{}`,  need to change in 6.0 to `maybeSeq && typeof maybeSet === 'object' && MAYBE_SET_SYMBOL in maybeSet`
...
class h {
  ...
  static _hex(i) {
    ...
    for (n = 0; n < 4; n += 1)
      for (s = n * 8, e = i[n], r = 0; r < 8; r += 2)
        (t[s + 1 + r] = a.charAt(e & 15)),
          (e >>>= 4),
          (t[s + 0 + r] = a.charAt(e & 15)),
          (e >>>= 4);
    return t.join("");
  }
  static _md5cycle(i, a) {
    let t = i[0],
      ...
      s = i[2],
      r = i[3];
    (t += (((e & s) | (~e & r)) + a[0] - 680876936) | 0),
      (t = (((t << 7) | (t >>> 25)) + e) | 0),
      (r += (((t & e) | (~t & s)) + a[1] - 389564586) | 0),
      (r = (((r << 12) | (r >>> 20)) + t) | 0),
      (s += (((r & t) | (~r & e)) + a[2] + 606105819) | 0),
      (s = (((s << 17) | (s >>> 15)) + r) | 0),
      (e += (((s & r) | (~s & t)) + a[3] - 1044525330) | 0),
      (e = (((e << 22) | (e >>> 10)) + s) | 0),
      (t += (((e & s) | (~e & r)) + a[4] - 176418897) | 0),
      (t = (((t << 7) | (t >>> 25)) + e) | 0),
      (r += (((t & e) | (~t & s)) + a[5] + 1200080426) | 0),
      (r = (((r << 12) | (r >>> 20)) + t) | 0),
      (s += (((r & t) | (~r & e)) + a[6] - 1473231341) | 0),
      (s = (((s << 17) | (s >>> 15)) + r) | 0),
      (e += (((s & r) | (~s & t)) + a[7] - 45705983) | 0),
      (e = (((e << 22) | (e >>> 10)) + s) | 0),
      (t += (((e & s) | (~e & r)) + a[8] + 1770035416) | 0),
      (t = (((t << 7) | (t >>> 25)) + e) | 0),
      (r += (((t & e) | (~t & s)) + a[9] - 1958414417) | 0),
      (r = (((r << 12) | (r >>> 20)) + t) | 0),
      (s += (((r & t) | (~r & e)) + a[10] - 42063) | 0),
      (s = (((s << 17) | (s >>> 15)) + r) | 0),
      (e += (((s & r) | (~s & t)) + a[11] - 1990404162) | 0),
      (e = (((e << 22) | (e >>> 10)) + s) | 0),
      (t += (((e & s) | (~e & r)) + a[12] + 1804603682) | 0),
      (t = (((t << 7) | (t >>> 25)) + e) | 0),
      (r += (((t & e) | (~t & s)) + a[13] - 40341101) | 0),
      (r = (((r << 12) | (r >>> 20)) + t) | 0),
      (s += (((r & t) | (~r & e)) + a[14] - 1502002290) | 0),
      (s = (((s << 17) | (s >>> 15)) + r) | 0),
      (e += (((s & r) | (~s & t)) + a[15] + 1236535329) | 0),
      (e = (((e << 22) | (e >>> 10)) + s) | 0),
      (t += (((e & r) | (s & ~r)) + a[1] - 165796510) | 0),
      (t = (((t << 5) | (t >>> 27)) + e) | 0),
      (r += (((t & s) | (e & ~s)) + a[6] - 1069501632) | 0),
      (r = (((r << 9) | (r >>> 23)) + t) | 0),
      (s += (((r & e) | (t & ~e)) + a[11] + 643717713) | 0),
      (s = (((s << 14) | (s >>> 18)) + r) | 0),
      (e += (((s & t) | (r & ~t)) + a[0] - 373897302) | 0),
      (e = (((e << 20) | (e >>> 12)) + s) | 0),
      (t += (((e & r) | (s & ~r)) + a[5] - 701558691) | 0),
      (t = (((t << 5) | (t >>> 27)) + e) | 0),
      (r += (((t & s) | (e & ~s)) + a[10] + 38016083) | 0),
      (r = (((r << 9) | (r >>> 23)) + t) | 0),
      (s += (((r & e) | (t & ~e)) + a[15] - 660478335) | 0),
      (s = (((s << 14) | (s >>> 18)) + r) | 0),
      (e += (((s & t) | (r & ~t)) + a[4] - 405537848) | 0),
      (e = (((e << 20) | (e >>> 12)) + s) | 0),
      (t += (((e & r) | (s & ~r)) + a[9] + 568446438) | 0),
      (t = (((t << 5) | (t >>> 27)) + e) | 0),
      (r += (((t & s) | (e & ~s)) + a[14] - 1019803690) | 0),
      (r = (((r << 9) | (r >>> 23)) + t) | 0),
      (s += (((r & e) | (t & ~e)) + a[3] - 187363961) | 0),
      (s = (((s << 14) | (s >>> 18)) + r) | 0),
      (e += (((s & t) | (r & ~t)) + a[8] + 1163531501) | 0),
      (e = (((e << 20) | (e >>> 12)) + s) | 0),
      (t += (((e & r) | (s & ~r)) + a[13] - 1444681467) | 0),
      (t = (((t << 5) | (t >>> 27)) + e) | 0),
      (r += (((t & s) | (e & ~s)) + a[2] - 51403784) | 0),
      (r = (((r << 9) | (r >>> 23)) + t) | 0),
      (s += (((r & e) | (t & ~e)) + a[7] + 1735328473) | 0),
      (s = (((s << 14) | (s >>> 18)) + r) | 0),
      (e += (((s & t) | (r & ~t)) + a[12] - 1926607734) | 0),
      (e = (((e << 20) | (e >>> 12)) + s) | 0),
      (t += ((e ^ s ^ r) + a[5] - 378558) | 0),
      (t = (((t << 4) | (t >>> 28)) + e) | 0),
      (r += ((t ^ e ^ s) + a[8] - 2022574463) | 0),
      (r = (((r << 11) | (r >>> 21)) + t) | 0),
      (s += ((r ^ t ^ e) + a[11] + 1839030562) | 0),
      (s = (((s << 16) | (s >>> 16)) + r) | 0),
      (e += ((s ^ r ^ t) + a[14] - 35309556) | 0),
      (e = (((e << 23) | (e >>> 9)) + s) | 0),
      (t += ((e ^ s ^ r) + a[1] - 1530992060) | 0),
      (t = (((t << 4) | (t >>> 28)) + e) | 0),
      (r += ((t ^ e ^ s) + a[4] + 1272893353) | 0),
      (r = (((r << 11) | (r >>> 21)) + t) | 0),
      (s += ((r ^ t ^ e) + a[7] - 155497632) | 0),
      (s = (((s << 16) | (s >>> 16)) + r) | 0),
      (e += ((s ^ r ^ t) + a[10] - 1094730640) | 0),
      (e = (((e << 23) | (e >>> 9)) + s) | 0),
      (t += ((e ^ s ^ r) + a[13] + 681279174) | 0),
      (t = (((t << 4) | (t >>> 28)) + e) | 0),
      (r += ((t ^ e ^ s) + a[0] - 358537222) | 0),
      (r = (((r << 11) | (r >>> 21)) + t) | 0),
      (s += ((r ^ t ^ e) + a[3] - 722521979) | 0),
      (s = (((s << 16) | (s >>> 16)) + r) | 0),
      (e += ((s ^ r ^ t) + a[6] + 76029189) | 0),
      (e = (((e << 23) | (e >>> 9)) + s) | 0),
      (t += ((e ^ s ^ r) + a[9] - 640364487) | 0),
      (t = (((t << 4) | (t >>> 28)) + e) | 0),
      (r += ((t ^ e ^ s) + a[12] - 421815835) | 0),
      (r = (((r << 11) | (r >>> 21)) + t) | 0),
      (s += ((r ^ t ^ e) + a[15] + 530742520) | 0),
      (s = (((s << 16) | (s >>> 16)) + r) | 0),
      (e += ((s ^ r ^ t) + a[2] - 995338651) | 0),
      (e = (((e << 23) | (e >>> 9)) + s) | 0),
      (t += ((s ^ (e | ~r)) + a[0] - 198630844) | 0),
      (t = (((t << 6) | (t >>> 26)) + e) | 0),
      (r += ((e ^ (t | ~s)) + a[7] + 1126891415) | 0),
      (r = (((r << 10) | (r >>> 22)) + t) | 0),
      (s += ((t ^ (r | ~e)) + a[14] - 1416354905) | 0),
      (s = (((s << 15) | (s >>> 17)) + r) | 0),
      (e += ((r ^ (s | ~t)) + a[5] - 57434055) | 0),
      (e = (((e << 21) | (e >>> 11)) + s) | 0),
      (t += ((s ^ (e | ~r)) + a[12] + 1700485571) | 0),
      (t = (((t << 6) | (t >>> 26)) + e) | 0),
      (r += ((e ^ (t | ~s)) + a[3] - 1894986606) | 0),
      (r = (((r << 10) | (r >>> 22)) + t) | 0),
      (s += ((t ^ (r | ~e)) + a[10] - 1051523) | 0),
      (s = (((s << 15) | (s >>> 17)) + r) | 0),
      (e += ((r ^ (s | ~t)) + a[1] - 2054922799) | 0),
      (e = (((e << 21) | (e >>> 11)) + s) | 0),
      (t += ((s ^ (e | ~r)) + a[8] + 1873313359) | 0),
      (t = (((t << 6) | (t >>> 26)) + e) | 0),
      (r += ((e ^ (t | ~s)) + a[15] - 30611744) | 0),
      (r = (((r << 10) | (r >>> 22)) + t) | 0),
      (s += ((t ^ (r | ~e)) + a[6] - 1560198380) | 0),
      (s = (((s << 15) | (s >>> 17)) + r) | 0),
      (e += ((r ^ (s | ~t)) + a[13] + 1309151649) | 0),
      (e = (((e << 21) | (e >>> 11)) + s) | 0),
      (t += ((s ^ (e | ~r)) + a[4] - 145523070) | 0),
      (t = (((t << 6) | (t >>> 26)) + e) | 0),
      (r += ((e ^ (t | ~s)) + a[11] - 1120210379) | 0),
      (r = (((r << 10) | (r >>> 22)) + t) | 0),
      (s += ((t ^ (r | ~e)) + a[2] + 718787259) | 0),
      (s = (((s << 15) | (s >>> 17)) + r) | 0),
      (e += ((r ^ (s | ~t)) + a[9] - 343485551) | 0),
      (e = (((e << 21) | (e >>> 11)) + s) | 0),
      (i[0] = (t + i[0]) | 0),
      (i[1] = (e + i[1]) | 0),
      (i[2] = (s + i[2]) | 0),
   ...
   */
  appendStr(i) {
    const a = this._buffer8,
      t = this._buffer32;
    let e = this._bufferLength,
      s,
      r;
    for (r = 0; r < i.length; r += 1) {
      if (((s = i.charCodeAt(r)), s < 128)) a[e++] = s;
      else if (s < 2048) (a[e++] = (s >>> 6) + 192), (a[e++] = (s & 63) | 128);
  ...
  appendAsciiStr(i) {
    ...
    let e = this._bufferLength,
      ...
      r = 0;
    for (;;) {
      for (s = Math.min(i.length - r, 64 - e); s--; ) a[e++] = i.charCodeAt(r++);
      if (e < 64) break;
  ...
  appendByteArray(i) {
    ...
    let e = this._bufferLength,
      ...
      r = 0;
    for (;;) {
      for (s = Math.min(i.length - r, 64 - e); s--; ) a[e++] = i[r++];
      if (e < 64) break;
      (this._dataLength += 64), h._md5cycle(this._state, t), (e = 0);
    }
    return (this._bufferLength = e), this;
  ...
  setState(i) {
    ...
    let s;
    for (
      this._dataLength = i.length,
        this._bufferLength = i.buflen,
        e[0] = t[0],
        e[1] = t[1],
        e[2] = t[2],
        e[3] = t[3],
   ...
   */
    ...
    if (
      ((t[a] = 128),
      (t[a + 1] = t[a + 2] = t[a + 3] = 0),
      e.set(h.buffer32Identity.subarray(s), s),
      a > 55 && (h._md5cycle(this._state, e), e.set(h.buffer32Identity)),
      r <= 4294967295)
    )
      e[14] = r;
    else {
  ...
  (function (exports$1) {
    ...
    function _preferredTypeLegacy(ext, type0, type1) {
      ...
      if (
        ...
        (score0 > score1 ||
          (score0 === score1 && exports$1.types[extension]?.slice(0, 12) === "application/"))
      ) {
        return type0;
      }
      return score0 > score1 ? type0 : type1;
...
class DiffContext extends Context {
  ...
  prepareDeltaResult(result) {
    ...
    if (typeof result === "object") {
      if (
        ((_a = this.options) === null || _a === void 0 ? void 0 : _a.omitRemovedValues) &&
      ...
      if ((_b = this.options) === null || _b === void 0 ? void 0 : _b.cloneDiffValues) {
        const clone$1 =
          typeof ((_c = this.options) === null || _c === void 0 ? void 0 : _c.cloneDiffValues) ===
          "function"
    ...
    for (y = 1; y < len2 + 1; y++) {
      if (match(array1, array2, x - 1, y - 1, context)) {
        ...
        matrixRowX[y] = Math.max(
          (_b = matrixRowBeforeX[y]) !== null && _b !== void 0 ? _b : 0,
          (_c = matrixRowX[y - 1]) !== null && _c !== void 0 ? _c : 0,
        );
      }
    }
  }
...
const diffFilter$3 = function arraysDiffFilter(context) {
  ...
  let detectMove = true;
  if (
    ((_c = context.options) === null || _c === void 0 ? void 0 : _c.arrays) &&
  ...
  if (
    (_e = (_d = context.options) === null || _d === void 0 ? void 0 : _d.arrays) === null ||
    _e === void 0
      ? void 0
      : _e.includeValueOnMove
  ) {
  ...
  for (index in delta) {
    if (index !== "_t") {
      if (index[0] === "_") {
        ...
        if (
          delta[removedOrMovedIndex] !== void 0 &&
          (delta[removedOrMovedIndex][2] === 0 || delta[removedOrMovedIndex][2] === ARRAY_MOVE)
        ) {
          toRemove.push(Number.parseInt(index.slice(1), 10));
        } else {
          throw new Error(
            `only removal or move can be applied at original array indices, invalid diff type: ${(_a = delta[removedOrMovedIndex]) === null || _a === void 0 ? void 0 : _a[2]}`,
          );
        }
      } else {
...
const objectsDiffFilter = (context) => {
  ...
  const propertyFilter =
    (_a = context.options) === null || _a === void 0 ? void 0 : _a.propertyFilter;
...
function getDiffMatchPatch(options, required) {
  ...
  if (!cachedDiffPatch) {
    let instance;
    if (
      (_a = options === null || options === void 0 ? void 0 : options.textDiff) === null ||
      _a === void 0
        ? void 0
        : _a.diffMatchPatch
    ) {
...
const diffFilter$1 = function textsDiffFilter(context) {
  ...
  const right = context.right;
  const minLength =
    ((_b = (_a = context.options) === null || _a === void 0 ? void 0 : _a.textDiff) === null ||
    _b === void 0
      ? void 0
      : _b.minLength) || DEFAULT_MIN_LENGTH;
...
class SessionPatcher {
  ...
  static applySessionDelta(sess, sessionDelta) {
    ...
    const parsedSession = applyDelta(sanitizedSession, sessionDelta.delta);
