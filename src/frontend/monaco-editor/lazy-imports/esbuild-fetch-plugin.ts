...
async function processCSS(css: string, baseURL: string, depth = 0): Promise<string> {
  ...
  while ((match = importRegex.exec(css)) !== null) {
    ...
    if (importUrlPath === undefined) continue;
    ...
    const cachedImport = urlCache.get(absoluteUrl);
    if (cachedImport !== undefined) {
      processedCss = processedCss.replace(match[0], cachedImport);
      continue;
    }
  ...
  for (const urlMatch of urlMatches) {
    ...
    if (urlPath && !urlPath.startsWith("data:")) {
      ...
      const cachedUrl = urlCache.get(absoluteUrl);
      if (cachedUrl !== undefined) {
        css = css.replace(fullMatchSyntax, cachedUrl);
        continue;
      }
