...
export function registerAuditQuestionnaireTools(
  ...
  registry.registerBuilt(
    t
      ...
      .handler(async ({ input, ctx }) => {
        return safeToolCall("audit_submit_evaluation", async () => {
  ...
  registry.registerBuilt(
    t
      ...
      .meta({ category: "persona", tier: "free" })
      .handler(async ({ input, ctx }) => {
        return safeToolCall("audit_get_results", async () => {
  ...
  registry.registerBuilt(
    t
      ...
      .meta({ category: "persona", tier: "free" })
      .handler(async ({ input, ctx }) => {
        return safeToolCall("audit_compare_personas", async () => {
          const results = await ctx.db
            .select()
            .from(personaAuditResults)
            .where(eq(personaAuditResults.batchId, input.batch_id));
          ...
          if (!best || !worst) {
            return jsonResult("No scored results available.", { error: "no_scored_results" });
          }
