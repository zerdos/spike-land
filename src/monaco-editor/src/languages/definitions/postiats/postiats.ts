...
export const language = <languages.IMonarchLanguage>{
	...
	// NOTE: staload and dynload are followed by special string literals containing {$IDENTIFIER}
	// variables and path punctuation (. / \). The Monarch tokenizer handles the {$IDENTIFIER}
	// interpolation in the lexing_DQUOTE state, but path punctuation is not specially highlighted.
	// Full support would require a dedicated staload/dynload string state.
	...
	// Effect annotations reference: https://github.com/githwxi/ATS-Postiats/blob/master/doc/BOOK/INT2PROGINATS/CODE/CHAP_EFFECTFUL/
	// These appear after :<effects> or =<effects> syntax in function signatures.
	keywords_effects: [
		'0', // no effects
		'fun',
		'clo',
	...
	tokenizer: {
		root: [
			...
			*/
			...
			// NOTE: a bare backtick not followed by '(' falls through to lexing_IDENT_sym below,
			// which will tokenize it as an operator via the @symbolic+ rule.
			...
			},
			{
				...
				action: {
					cases: {
						// NOTE: dynload/staload strings have special path syntax (/ \ .) and
						// {$IDENTIFIER} interpolation. The {$IDENTIFIER} part is handled in
						// lexing_DQUOTE. Path punctuation is not separately highlighted —
						// a dedicated state would be needed for full fidelity.
			...
			// lexing_ZERO / lexing_INT_hex:
			// Handles 0x hex literals (with optional float exponent) and octal literals.
			// The hex float path (0x...p±N) is handled by the hexiexp/fexponent_bin rules below.
			// Plain zero (0) falls through to the decimal integer rule.
		...
		lexing_COMMENT_rest: [
			// The Monarch docs note that $ (end-of-line) may not match in all tokenizer states.
			// The /.*/ rule below handles the rest-of-line content, and @pop returns to root
			// at the next line boundary, which is the correct behavior for //// line comments.
			[/$/, 'comment', '@pop'],
			[/.*/, 'comment']
		],
