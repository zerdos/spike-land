...
(function () {
	...
	const npmLsOutput = JSON.parse(
		child_process
			.execSync('npm ls typescript --depth=0 --json=true', { cwd: REPO_ROOT })
			.toString()
	);
...
export const typescriptVersion = "${typeScriptDependencyVersion}";\n`
	...
	// These .replace() calls inject CodeQL suppression comments into TypeScript's compiled JS output.
	// They match exact string literals, so they will silently no-op if TypeScript changes the code.
	// The guard below ensures CI fails loudly if the patterns stop matching after a TS version bump.
	const codeqlPatterns: Array<[search: string, replacement: string]> = [
		[
	...
	for (const [search, replacement] of codeqlPatterns) {
		const before = tsServices;
		tsServices = tsServices.replace(search, replacement);
		if (tsServices === before) {
			throw new Error(
				`CodeQL suppression pattern not found in typescript.js — the TypeScript output may have changed.\n` +
					`  Pattern: ${JSON.stringify(search)}\n` +
					`  Update the patterns in build/importTypescript.ts to match the new output.`
			);
		}
	}
