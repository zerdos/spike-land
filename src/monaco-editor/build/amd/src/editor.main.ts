		...
		if (label === 'json') {
			return new Worker(
				getWorkerBootstrapUrl(
					/// @ts-ignore
					new URL(
						'../../../src/languages/features/json/json.worker.ts?esm',
						import.meta.url
					)
				)
			);
		...
		if (label === 'css' || label === 'scss' || label === 'less') {
			return new Worker(
				getWorkerBootstrapUrl(
					/// @ts-ignore
					new URL(
						'../../../src/languages/features/css/css.worker.ts?esm',
						import.meta.url
					)
				)
			);
		...
		if (label === 'html' || label === 'handlebars' || label === 'razor') {
			return new Worker(
				getWorkerBootstrapUrl(
					/// @ts-ignore
					new URL(
						'../../../src/languages/features/html/html.worker.ts?esm',
						import.meta.url
					)
				)
			);
		...
		if (label === 'typescript' || label === 'javascript') {
			return new Worker(
				getWorkerBootstrapUrl(
					/// @ts-ignore
					new URL(
						'../../../src/languages/features/typescript/ts.worker.ts?esm',
						import.meta.url
					)
				)
			);
