...
/**
 * AMD-specific module ID mapping.
 * Differs from ESM: src/deprecated maps to '.' (not 'vs/') and src/ maps to '.' (not 'vs/').
 * This matches the AMD output structure where all files go under the 'vs/' AMD base path.
 *
 * @param {string} moduleId
 * @param {string} newExt (with leading .)
 * @returns {string | undefined}
