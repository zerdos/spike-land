...
import { ensureSuspenseTimers, fetchOptimistic, shouldSuspend } from "./suspense";
...
export function useQueries<T extends Array<any>, TCombinedResult = QueriesResults<T>>(
  ...
  const defaultedQueries = React.useMemo(
    () =>
      queries.map((opts) => {
        const defaultedOptions = client.defaultQueryOptions(opts as QueryObserverOptions);
  ...
  React.useEffect(() => {
    observer.setQueries(defaultedQueries, options as QueriesObserverOptions<TCombinedResult>);
