...
test("object freezing", async () => {
  ...
  expect(
    Object.isFrozen(z.object({ a: z.string(), 1: z.number() }).readonly().parse({ a: "b", 1: 2 })),
  ).toBe(true);
  expect(
    Object.isFrozen(
      await z
  ...
  expect(
    Object.isFrozen(await z.promise(z.string()).readonly().parseAsync(Promise.resolve("a"))),
  ).toBe(true);
});
...
test("async object freezing", async () => {
  ...
  expect(
    Object.isFrozen(await z.tuple([z.string(), z.number()]).readonly().parseAsync(["a", 1])),
  ).toBe(true);
  expect(
    Object.isFrozen(
      await z
  ...
  expect(
    Object.isFrozen(await z.record(z.string(), z.string()).readonly().parseAsync({ a: "b" })),
  ).toBe(true);
  expect(
    Object.isFrozen(await z.record(z.string(), z.number()).readonly().parseAsync({ a: 1 })),
  ).toBe(true);
  expect(
    Object.isFrozen(
      await z.object({ a: z.string(), 1: z.number() }).readonly().parseAsync({ a: "b", 1: 2 }),
    ),
  ).toBe(true);
  expect(
    Object.isFrozen(await z.promise(z.string()).readonly().parseAsync(Promise.resolve("a"))),
