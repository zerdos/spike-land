  ...
  const prepareMock = vi.fn().mockImplementation((sql: string) => {
    ...
    return {
      ...
      first: overrides.first
        ? vi.fn().mockImplementation(overrides.first)
        : vi.fn().mockResolvedValue(null),
      all: overrides.all
        ? vi.fn().mockImplementation(overrides.all)
        : vi.fn().mockResolvedValue({ results: [] }),
      run: overrides.run
        ? vi.fn().mockImplementation(overrides.run)
...
function defaultDB(
  ...
  return buildDB((sql) => ({
    first: async () => {
      ...
      if (sql.includes("bug_reports WHERE bug_id") && sql.includes("reporter_id"))
        return opts.existingReport ?? null;
      if (sql.includes("SELECT * FROM bugs") || sql.includes("WHERE id = ?"))
        return opts.bugRow ?? null;
      ...
      if (sql.includes("elo, report_count FROM bugs WHERE id"))
        return { elo: 1200, report_count: 1 };
      if (sql.includes("RETURNING id")) return { id: "new-bug-id" };
      return null;
    },
    all: async () => {
      if (sql.includes("bugs WHERE 1=1") || sql.includes("SELECT id, title"))
        return { results: bugs };
      return { results: [] };
    },
  }));
  ...
  it("increments existing bug on duplicate report", async () => {
    ...
    const db = buildDB((sql) => ({
      first: async () => {
        ...
        if (sql.includes("elo, report_count FROM bugs WHERE id"))
          return { elo: 1200, report_count: 2 };
        return null;
      },
      all: async () => ({ results: [] }),
    }));
...
describe("bugbook — POST /bugbook/:id/confirm", () => {
  ...
  it("returns 409 when user already reported bug", async () => {
    const db = buildDB((sql) => ({
      first: async () => {
        if (sql.includes("SELECT * FROM bugs"))
          return { id: "b1", category: "svc", severity: "medium" };
        if (sql.includes("bug_reports WHERE bug_id") && sql.includes("reporter_id"))
