...
} from "../../../src/edge-api/main/api/lib/cache-version.js";
...
describe("getCacheVersion", () => {
  ...
  it("extracts build-sha from index.html", async () => {
    const r2 = mockR2(
      makeR2Object(
        '<html><head><meta name="build-sha" content="abcdef1234567890" /></head><body></body></html>',
      ),
    );
  ...
  it("caches the extracted version and respects TTL", async () => {
    const r2 = mockR2(
      makeR2Object(
        '<html><head><meta name="build-sha" content="initial12345678" /></head><body></body></html>',
      ),
    );
    ...
    r2.get = vi
      ...
      .mockResolvedValue(
        makeR2Object(
          '<html><head><meta name="build-sha" content="changed12345678" /></head><body></body></html>',
        ),
      );
  ...
  it("falls back to timestamp string if index.html is missing or has no build-sha", async () => {
    ...
    const noShaR2 = mockR2(
      makeR2Object("<html><head><title>No SHA</title></head><body></body></html>"),
    );
