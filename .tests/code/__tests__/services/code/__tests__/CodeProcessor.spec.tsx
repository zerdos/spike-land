...
describe("CodeProcessor", () => {
  ...
  describe("runCode", () => {
    ...
    it("should throw error if rendering fails", async () => {
      ...
      vi.mocked(RenderService.prototype.handleRender).mockRejectedValue(new Error("Render failed"));

      await expect(codeProcessor.runCode(mockTranspiled)).rejects.toThrow("Render failed");
    });
  });
});
