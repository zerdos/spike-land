...
import {
  ...
  sendGA4Events,
  type GA4Event,
} from "../../../src/edge-api/spike-land/core-logic/lib/ga4";
...
describe("sendGA4Events", () => {
  ...
  it("skips sending when GA_MEASUREMENT_ID is missing", async () => {
    ...
    await sendGA4Events({ GA_MEASUREMENT_ID: "", GA_API_SECRET: "secret" }, "client-1", [
  ...
  it("skips sending when GA_API_SECRET is missing", async () => {
    ...
    await sendGA4Events({ GA_MEASUREMENT_ID: "G-TEST", GA_API_SECRET: "" }, "client-1", [
      { name: "test_event", params: { key: "value" } },
    ]);
  ...
  it("truncates event name to 40 characters", async () => {
    ...
    await sendGA4Events({ GA_MEASUREMENT_ID: "G-TEST", GA_API_SECRET: "secret" }, "client-1", [
