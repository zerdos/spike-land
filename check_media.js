      ...
      const results = await Promise.all(elements.map(el => {
        return new Promise(resolve => {
          if (el.readyState >= 1) { // HAVE_METADATA
            ...
            el.addEventListener('loadedmetadata', () => resolve({ src: el.currentSrc || el.src, ready: true }), { once: true });
            el.addEventListener('error', () => resolve({ src: el.currentSrc || el.src, ready: false, error: el.error }), { once: true });
            
            // Timeout in case it hangs
            setTimeout(() => resolve({ src: el.currentSrc || el.src, ready: false, error: 'timeout' }), 3000);
          }
        });
      }));
